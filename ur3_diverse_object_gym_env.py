...
class Ur3DiverseObjectEnv(ur3GymEnv.Ur3GymEnv):
  ...
  def reset(self):
    ...
    p.setGravity(0, 0, -10)
    robot_path = os.path.join(os.path.dirname(__file__)) + "/pybullet_data"
    self._kuka = ur3.Kuka(urdfRootPath=robot_path, timeStep=self._timeStep)
    self._envStepCounter = 0
    p.stepSimulation()
  ...
  def _randomly_place_objects(self, urdfList):
    ...
    for urdf_name in urdfList:
      ...
      uid = p.loadURDF(urdf_path, [xpos, ypos, .03], [orn[0], orn[1], orn[2], orn[3]])
      objectUids.append(uid)
      # Let each object fall to the tray individual, to prevent object intersection.
      for _ in range(500):
        p.stepSimulation()
    return objectUids
  ...
  def _reward(self):
    ...
    The reward is 1 if one of the objects is above height .2 at the end of the episode.
    """
    reward = 0
